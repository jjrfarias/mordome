...
- Grupo de ingrediente pertence a um produto (não é reutilizável entre produtos nesta fatia, ver ADR 0022) e agrupa opções escolhidas manualmente pelo cliente/atendente no momento da venda — diferente da ficha técnica, que é consumo automático e sem escolha. A opção pode somar um `priceDelta` (sempre ≥ 0 nesta fatia) ao preço unitário do item vendido. `SaleItem`/`TabItem` guardam um retrato (`selectedOptionsSnapshot`) das opções escolhidas, no mesmo padrão de `recipeSnapshot`.
