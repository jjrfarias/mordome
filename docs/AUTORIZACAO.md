...
`reports.time_by_status.view`). Isso permite que um perfil enxergue "Vendas por período" sem ter
acesso a "Faturamento por dia", "Desempenho por atendente/garçom", "Vendas por forma de pagamento",
"Vendas por área de entrega", "Itens vendidos", "Itens consumidos", "Tempo de produção" ou "Tempo
por status", e vice-versa — decisão de produto explícita do dono.
