...
## Atualização — roteiros de apresentação

Os roteiros aprovados estão em `docs/ROTEIROS-VIDEOS.md`. Existem duas peças independentes: vídeo genérico do SaaS Mordomê, com identidade padrão verde/areia/terracota e até 3 minutos; e vídeo **Mordomê para Família Betão**, com personalização vermelha/creme/dourada e até 1 minuto. Ambos apresentam a visão completa, incluindo recursos planejados representados por mockups. A continuidade opcional Nuvem + Local deve usar a mensagem **“A internet caiu. Sua operação, não.”** e demonstrar operação local seguida de sincronização.
