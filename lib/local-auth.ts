...
export async function getLocalSession() {
  ...
  return {
    ...
    canManageFloor: localUser?.effectivePermissionKeys.includes("floor.manage") ?? true,
    canOperateDelivery: localUser?.effectivePermissionKeys.includes("delivery.operate") ?? true,
    canDeliverOrders: localUser?.effectivePermissionKeys.includes("delivery.deliver") ?? true,
    ...
    canManageFinance: localUser?.effectivePermissionKeys.includes("finance.manage") ?? true,
    canManageFinanceEntries: localUser?.effectivePermissionKeys.includes("finance.entries.manage") ?? true,
    canViewFinanceCashflow: localUser?.effectivePermissionKeys.includes("finance.cashflow.view") ?? true,
    canManageSettlements: localUser?.effectivePermissionKeys.includes("settlements.manage") ?? true,
    ...
    canManageCustomers: localUser?.effectivePermissionKeys.includes("customers.manage") ?? true,
    canManageFiscal: localUser?.effectivePermissionKeys.includes("fiscal.manage") ?? true,
...
export function createLocalEstablishment(name: string, slug: string) {
  ...
  const establishment = { id: `${slug}-${Date.now()}`, name, slug, active: true, ...blankAddress, ...blankStorefront };
  localEstablishments.push(establishment);
  return { ...establishment };
}

export function updateLocalEstablishment(id: string, data: { name?: string; slug?: string; active?: boolean } & Partial<LocalEstablishmentAddress> & Partial<LocalEstablishmentStorefront>) {
