...
type TabItem = { id: string; productId: string | null; productName: string; quantity: number; sentQuantity: number; unitPrice: number; active: boolean; selectedOptionsSnapshot?: SelectedIngredientOption[] | null };
type Order = { id: string; status: "RECEIVED" | "PREPARING" | "READY" | "DELIVERED" | "CANCELLED"; sentAt: string; items: { id?: string; productName: string; quantity: number; cancelledQuantity?: number; stationId?: string | null; selectedOptionsSnapshot?: SelectedIngredientOption[] | null }[]; tableNumber: number; tabId: string; isCounter?: boolean };
const orderLabel = (order: Order) => order.isCounter ? "Balcão" : `Mesa ${String(order.tableNumber).padStart(2, "0")}`;
...
function CommandView({ table, products, query, setQuery, category, setCategory, saving, error, checkout, setCheckout, canCancelSentItems, cancelTarget, setCancelTarget, onBack, onAdd, onQuantity, onCancelSent, onSend, onClose }: { table: Table; products: Product[]; query: string; setQuery: (value: string) => void; category: string; setCategory: (value: string) => void; saving: boolean; error: string; checkout: boolean; setCheckout: (value: boolean) => void; canCancelSentItems: boolean; cancelTarget: TabItem | null; setCancelTarget: (item: TabItem | null) => void; onBack: () => void; onAdd: (product: Product, optionSelections?: { groupId: string; optionIds: string[] }[]) => Promise<boolean>; onQuantity: (item: TabItem, quantity: number) => Promise<boolean>; onCancelSent: (item: TabItem, quantity: number, reason: string) => Promise<boolean>; onSend: () => Promise<boolean>; onClose: (checkout: SaleCheckout) => Promise<boolean> }) {
  ...
  const addProduct = (product: Product) => { if (productHasIngredientChoices(product)) { setPickerProduct(product); return; } void onAdd(product); };
  return <div className="command-layout"><section className="catalog"><button className="back" onClick={onBack}><ChevronLeft/> Voltar ao salão</button><div className="search"><Search/><input placeholder="Buscar produto..." value={query} onChange={event => setQuery(event.target.value)}/></div><div className="chips">{categories.map(item => <button key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div>{error && <div className="auth-error">{error}</div>}<div className="product-grid">{visible.map(product => <button className="product-card" disabled={saving} key={product.id} onClick={() => addProduct(product)}><span>{product.imageUrl ? <img src={product.imageUrl} alt="" /> : "🌭"}</span><div><small>{product.category}</small><b>{product.name}</b><strong>{money(product.price)}</strong></div><Plus/></button>)}</div></section><aside className="ticket"><div className="ticket-title"><div><span>COMANDA</span><h2>Mesa {String(table.number).padStart(2, "0")}</h2></div><span className="status-badge">{table.tab ? "Aberta" : "Nova"}</span></div><div className="ticket-items">{items.length === 0 ? <div className="empty"><ShoppingBag/><b>Comanda vazia</b><span>Adicione o primeiro produto para abrir a comanda.</span></div> : items.map(item => <div className="ticket-item" key={item.id}><span className="food">🌭</span><div><b>{item.productName}</b>{item.selectedOptionsSnapshot?.length ? <small className="pos-cart-options">{item.selectedOptionsSnapshot.map(option => option.optionName).join(", ")}</small> : null}<small>{money(item.unitPrice)}{item.sentQuantity > 0 ? ` · ${item.sentQuantity} enviado(s)` : ""}</small>{canCancelSentItems && item.sentQuantity > 0 && <button className="cancel-sent-item" disabled={saving} onClick={() => setCancelTarget(item)}>Cancelar enviado</button>}</div><div className="stepper"><button disabled={saving || item.quantity <= item.sentQuantity} onClick={() => void onQuantity(item, item.quantity - 1)}>{item.quantity === 1 ? <X/> : <Minus/>}</button><b>{item.quantity}</b><button disabled={saving} onClick={() => void onQuantity(item, item.quantity + 1)}><Plus/></button></div></div>)}</div><div className="ticket-footer"><div><span>Subtotal</span><b>{money(total)}</b></div><div><span>Serviço (10%)</span><b>{money(service)}</b></div><div className="grand"><span>Total</span><b>{money(total + service)}</b></div>{pending ? <button className="primary wide" disabled={saving} onClick={() => void onSend()}><ChefHat/> Enviar para cozinha</button> : <button className="secondary wide" disabled={!items.length || saving} onClick={() => setCheckout(true)}><CircleDollarSign/> Fechar conta</button>}</div></aside>{checkout && <Checkout total={total + service} onCancel={() => setCheckout(false)} onConfirm={onClose}/>} {cancelTarget && <CancelSentItem item={cancelTarget} saving={saving} onClose={() => setCancelTarget(null)} onConfirm={async (quantity, reason) => { const ok = await onCancelSent(cancelTarget, quantity, reason); if (ok) setCancelTarget(null); return ok; }}/>} {pickerProduct && <IngredientPicker product={pickerProduct} onClose={() => setPickerProduct(null)} onConfirm={(_unitPrice, _selectedOptions, optionSelections) => { void onAdd(pickerProduct, optionSelections); setPickerProduct(null); }} />}</div>;
}
...
function KitchenView({ establishmentId, establishmentName, orders, stations, saving, error, canReprint, onToast, onAdvance }: { establishmentId: string; establishmentName: string; orders: Order[]; stations: Station[]; saving: boolean; error: string; canReprint: boolean; onToast: (message: string) => void; onAdvance: (order: Order) => Promise<void> }) {
  ...
  const reprintOrder = async (order: Order) => {
    ...
    try {
      ...
      for (const ticket of tickets) printKitchenOrder({ establishmentName, stationName: ticket.station.name, label: orderLabel(order), orderId: order.id, sentAt: order.sentAt, items: ticket.items.map(item => ({ name: item.selectedOptionsSnapshot?.length ? `${item.productName} — ${item.selectedOptionsSnapshot.map(option => option.optionName).join(", ")}` : item.productName, quantity: item.quantity - (item.cancelledQuantity ?? 0) })) });
  ...
  return <div className="page-content">
    ...
    {filteredOrders.length === 0 ? <div className="big-empty"><ChefHat/><h2>Tudo em dia por aqui</h2><p>Novos pedidos enviados pelo salão e pelo PDV aparecerão nesta tela.</p></div> : <div className="kds-grid">{filteredOrders.map(order => <article className={`kds-card ${statusLabels[order.status].replace(" ", "-").toLowerCase()}`} key={order.id}><div className="kds-head"><div><span>{order.isCounter ? "PDV" : "MESA"}</span><b>{order.isCounter ? "Balcão" : String(order.tableNumber).padStart(2, "0")}</b></div><span><Clock3/> {new Date(order.sentAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</span></div><div className="kds-items">{order.items.map((item, index) => <div className={item.cancelledQuantity ? "cancelled" : ""} key={item.id ?? `${order.id}-${index}`}><b>{Math.max(0, item.quantity - (item.cancelledQuantity ?? 0))}×</b><span>{item.productName}{item.selectedOptionsSnapshot?.length ? <small className="kds-item-options">{item.selectedOptionsSnapshot.map(option => option.optionName).join(", ")}</small> : null}{item.cancelledQuantity ? <small>{item.cancelledQuantity} cancelado(s)</small> : null}</span></div>)}</div><div className="kds-foot"><span>{statusLabels[order.status]}</span>{nextStatus[order.status] && <button disabled={saving} onClick={() => void onAdvance(order)}>{order.status === "READY" ? "Entregar" : order.status === "PREPARING" ? "Marcar pronto" : "Iniciar preparo"}</button>}</div></article>)}</div>}
  </div>;
}
