...
type Order = { id: string; status: "RECEIVED" | "PREPARING" | "READY" | "DELIVERED" | "CANCELLED"; sentAt: string; items: { id?: string; productName: string; quantity: number; cancelledQuantity?: number; stationId?: string | null; selectedOptionsSnapshot?: SelectedIngredientOption[] | null }[]; tableNumber: number; tabId: string };
...
function KitchenView({ establishmentId, establishmentName, orders, stations, saving, error, canReprint, onToast, onAdvance }: { establishmentId: string; establishmentName: string; orders: Order[]; stations: Station[]; saving: boolean; error: string; canReprint: boolean; onToast: (message: string) => void; onAdvance: (order: Order) => Promise<void> }) {
  ...
  const reprintOrder = async (order: Order) => {
    ...
    try {
      ...
      for (const ticket of tickets) printKitchenOrder({ establishmentName, stationName: ticket.station.name, table: order.tableNumber, orderId: order.id, sentAt: order.sentAt, items: ticket.items.map(item => ({ name: item.selectedOptionsSnapshot?.length ? `${item.productName} — ${item.selectedOptionsSnapshot.map(option => option.optionName).join(", ")}` : item.productName, quantity: item.quantity - (item.cancelledQuantity ?? 0) })) });
