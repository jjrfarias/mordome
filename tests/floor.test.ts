...
test("primeiro item abre comanda e mantém salão isolado por unidade", () => {
  ...
  const result = addLocalTabItem({ establishmentId: unitA, tableId: table.id, operatorId: "waiter-a", product: { id: "product-1", name: "Cachorro-quente", price: 20 } });
...
test("rodadas enviadas à cozinha preservam itens e bloqueiam redução já enviada", () => {
  ...
  const added = addLocalTabItem({ establishmentId: unit, tableId: table.id, operatorId: "waiter", product: { id: "product-2", name: "Batata", price: 12 } });
...
test("fechamento vincula venda e libera a mesa", () => {
  ...
  const added = addLocalTabItem({ establishmentId: unit, tableId: table.id, operatorId: "waiter", product: { id: "product-3", name: "Suco", price: 8 } });
  assert.notEqual(added, "TABLE_NOT_FOUND"); if (added === "TABLE_NOT_FOUND") return;
...
test("cancelamento justificado reduz item enviado e avisa a cozinha", () => {
  ...
  const first = addLocalTabItem({ establishmentId: unit, tableId: table.id, operatorId: "waiter", product: { id: "product-cancel", name: "Porção", price: 18 } });
  assert.notEqual(first, "TABLE_NOT_FOUND"); if (first === "TABLE_NOT_FOUND") return;
  addLocalTabItem({ establishmentId: unit, tableId: table.id, operatorId: "waiter", product: { id: "product-cancel", name: "Porção", price: 18 } });
