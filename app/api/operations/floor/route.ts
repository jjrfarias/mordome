...
async function loadTables(establishmentId: string, viewer: { userId: string; canManageFloor: boolean }) {
  return db.diningTable.findMany({ where: { establishmentId, active: true, isCounter: false, ...(viewer.canManageFloor ? {} : { OR: [{ assignedWaiterId: null }, { assignedWaiterId: viewer.userId }] }) }, include: floorInclude, orderBy: { number: "asc" } });
}

// Pedidos do PDV (ADR 0044): a mesa virtual "Balcão" (`isCounter: true`) pode ter dezenas de
// comandas abertas ao mesmo tempo (uma por venda), o que não cabe no formato "1 comanda aberta por
// mesa" de `floorInclude`/`loadTables` — por isso uma consulta separada, só para a tela de Cozinha.
async function loadCounterOrders(establishmentId: string) {
...
async function changeOrderStatus(session: Session, data: Extract<z.infer<typeof actionSchema>, { action: "CHANGE_ORDER_STATUS" }>, request: Request) {
  ...
  await db.$transaction(async tx => { const order = await tx.order.findFirst({ where: { id: data.orderId, tab: { establishmentId: session.establishment.id, status: "OPEN" } }, include: { tab: { include: { table: true } } } }); if (!order) throw new Error("ORDER_NOT_FOUND"); if (next[order.status] !== data.status) throw new Error("INVALID_ORDER_TRANSITION"); const label = order.tab.table.isCounter ? "Balcão" : `mesa ${order.tab.table.number}`; const updated = await tx.order.update({ where: { id: order.id }, data: { status: data.status } }); await tx.orderStatusHistory.create({ data: { orderId: order.id, status: data.status, actorId: session.user.id } }); await tx.auditEvent.create({ data: { organizationId: session.organization.id, establishmentId: session.establishment.id, actorId: session.user.id, action: "ORDER_STATUS_CHANGE", entityType: "Order", entityId: order.id, reason: `Etapa do pedido alterada — ${label}`, before: { status: order.status }, after: { status: updated.status, tabId: order.tabId, tableNumber: order.tab.table.number }, ...requestAuditMetadata(request) } }); }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
}
