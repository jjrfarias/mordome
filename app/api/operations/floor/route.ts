...
const actionSchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("ADD_ITEM"), tableId: z.string().min(1), productId: z.string().min(1) }),
...
const floorInclude = {
  tabs: { where: { status: "OPEN" as const }, take: 1, include: { openedBy: { select: { id: true, name: true, username: true } }, items: { where: { active: true }, orderBy: { createdAt: "asc" as const } }, orders: { orderBy: { sentAt: "asc" as const }, include: { sentBy: { select: { id: true, name: true, username: true } }, items: { include: { cancellations: true, tabItem: { select: { productId: true } } } }, statusHistory: { include: { actor: { select: { id: true, name: true, username: true } } }, orderBy: { createdAt: "asc" as const } } } } } },
...
function serializeTables(tables: Awaited<ReturnType<typeof loadTables>>, byProduct: Map<string, string>) {
  return tables.map(table => { const tab = table.tabs[0]; return { id: table.id, number: table.number, seats: table.seats, name: table.name, active: table.active, tab: tab ? { ...tab, items: tab.items.map(item => ({ ...item, quantity: Number(item.quantity), sentQuantity: Number(item.sentQuantity), unitPrice: Number(item.unitPrice) })), orders: tab.orders.map(order => ({ ...order, items: order.items.map(item => ({ ...item, quantity: Number(item.quantity), stationId: byProduct.get(item.tabItem.productId ?? "") ?? null, cancelledQuantity: item.cancellations.reduce((sum, cancellation) => sum + Number(cancellation.quantity), 0), cancellations: item.cancellations.map(cancellation => ({ ...cancellation, quantity: Number(cancellation.quantity) })) })) })) } : null }; });
}

async function loadTables(establishmentId: string) {
  return db.diningTable.findMany({ where: { establishmentId, active: true }, include: floorInclude, orderBy: { number: "asc" } });
}

export async function GET(request: Request) {
  if (!isSameOrigin(request)) return Response.json({ error: "Origem inválida." }, { status: 403 });
  if (isLocalAuthEnabled()) { const session = await getLocalSession(); if (!session) return Response.json({ error: "Não autenticado." }, { status: 401 }); if (!session.canOperateFloor) return Response.json({ error: "Acesso negado ao salão." }, { status: 403 }); return Response.json(getLocalFloor(session.establishment.id)); }
  ...
  const tables = serializeTables(await loadTables(session.establishment.id), byProduct);
...
async function addItem(session: Session, data: Extract<z.infer<typeof actionSchema>, { action: "ADD_ITEM" }>, request: Request) {
  await db.$transaction(async tx => { const table = await tx.diningTable.findFirst({ where: { id: data.tableId, establishmentId: session.establishment.id, active: true } }); if (!table) throw new Error("TABLE_NOT_FOUND"); const product = await tx.product.findFirst({ where: { id: data.productId, organizationId: session.organization.id, active: true }, include: { variants: { where: { isDefault: true, active: true }, take: 1, include: { offerings: { where: { establishmentId: session.establishment.id, channel: "FLOOR", active: true }, take: 1 } } } } }); const offering = product?.variants[0]?.offerings[0]; if (!product || !offering) throw new Error("PRODUCT_NOT_AVAILABLE"); let tab = await tx.tab.findFirst({ where: { tableId: table.id, establishmentId: session.establishment.id, status: "OPEN" } }); if (!tab) { tab = await tx.tab.create({ data: { establishmentId: session.establishment.id, tableId: table.id, openedById: session.user.id } }); await tx.auditEvent.create({ data: { organizationId: session.organization.id, establishmentId: session.establishment.id, actorId: session.user.id, action: "TAB_OPEN", entityType: "Tab", entityId: tab.id, reason: `Comanda aberta na mesa ${table.number}`, after: { tableId: table.id, tableNumber: table.number }, ...requestAuditMetadata(request) } }); } const current = await tx.tabItem.findFirst({ where: { tabId: tab.id, productId: product.id, active: true } }); const item = current ? await tx.tabItem.update({ where: { id: current.id }, data: { quantity: { increment: 1 } } }) : await tx.tabItem.create({ data: { tabId: tab.id, productId: product.id, productName: product.name, quantity: 1, unitPrice: offering.price, addedById: session.user.id } }); await tx.auditEvent.create({ data: { organizationId: session.organization.id, establishmentId: session.establishment.id, actorId: session.user.id, action: "TAB_ITEM_CHANGE", entityType: "TabItem", entityId: item.id, reason: `${product.name} adicionado à mesa ${table.number}`, before: { quantity: Number(current?.quantity ?? 0) }, after: { quantity: Number(item.quantity), productId: product.id, productName: product.name, unitPrice: Number(offering.price), tabId: tab.id, tableNumber: table.number }, ...requestAuditMetadata(request) } }); }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
}
