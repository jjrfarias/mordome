...
export async function GET(request: Request) {
  ...
  if (isLocalAuthEnabled()) {
    ...
    const products = listLocalCatalog(session.establishment.id).filter(product => product.active && product.channels.includes(parsed.data.channel));
  ...
  return Response.json({ products: offerings.map(offering => ({ id: offering.variant.product.id, name: offering.variant.product.name, category: offering.variant.product.category?.name ?? "Sem categoria", price: Number(offering.price), active: true })) });
}
