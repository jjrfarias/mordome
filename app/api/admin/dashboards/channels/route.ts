...
import { DASHBOARDS_CHANNELS_VIEW } from "@/lib/permissions";
import { buildChannelDailyRevenue, buildChannelKpis, rankChannels } from "@/lib/dashboards/channels";
import type { SaleRecord } from "@/lib/reports/sales";
import { defaultMonthRange } from "@/lib/cashflow";

// Dashboard "Canais" (ADR 0043): diferente de "Acompanhamento de vendas" e "Acompanhamento de
// vendas multilojas" (ADR 0042, sempre UM dia), este olha para um INTERVALO de dias — mesmo
// contrato `?from=&to=` já usado pelos relatórios (ver `/api/admin/reports/payment-methods`).
...
export async function GET(request: Request) {
  ...
  if (isLocalAuthEnabled()) {
    ...
    if (!session.permissionKeys.includes(DASHBOARDS_CHANNELS_VIEW)) return Response.json({ error: "Acesso negado." }, { status: 403 });
    ...
    const kpis = buildChannelKpis(sales);
    const { channels, points } = buildChannelDailyRevenue(sales);
    return Response.json({ kpis, ranking: rankChannels(kpis), channels, points, from: fromDate.toISOString(), to: toDate.toISOString() });
  }
  ...
  const kpis = buildChannelKpis(records);
  const { channels, points } = buildChannelDailyRevenue(records);
  return Response.json({ kpis, ranking: rankChannels(kpis), channels, points, from: fromDate.toISOString(), to: toDate.toISOString() });
}
