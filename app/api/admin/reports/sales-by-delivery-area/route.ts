...
import { REPORTS_SALES_BY_DELIVERY_AREA_VIEW } from "@/lib/permissions";
import { buildSalesByDeliveryAreaRows, summarizeSalesByDeliveryArea } from "@/lib/reports/sales-by-delivery-area";
...
export async function GET(request: Request) {
  ...
  if (isLocalAuthEnabled()) {
    ...
    if (!session.permissionKeys.includes(REPORTS_SALES_BY_DELIVERY_AREA_VIEW)) return Response.json({ error: "Acesso negado." }, { status: 403 });
    ...
    const rows = buildSalesByDeliveryAreaRows(sales);
    return Response.json({ rows, summary: summarizeSalesByDeliveryArea(rows), from: fromDate.toISOString(), to: toDate.toISOString() });
  }
  ...
  const sales = await db.sale.findMany({
    where: { organizationId: session.organization.id, establishmentId: session.establishment.id, channel: "DELIVERY", status: { in: ["COMPLETED", "PARTIALLY_REFUNDED"] }, completedAt: { gte: fromDate, lte: toDate } },
    include: { payments: true, refunds: true, deliveryOrder: { include: { deliveryArea: true } } },
  ...
  const records: SaleRecord[] = sales.map(sale => ({
    ...
    deliveryAreaId: sale.deliveryOrder?.deliveryAreaId ?? null,
    deliveryAreaName: sale.deliveryOrder?.deliveryArea?.name ?? null,
    deliveryFee: Number(sale.deliveryFee),
  }));

  const rows = buildSalesByDeliveryAreaRows(records);
  return Response.json({ rows, summary: summarizeSalesByDeliveryArea(rows), from: fromDate.toISOString(), to: toDate.toISOString() });
}
