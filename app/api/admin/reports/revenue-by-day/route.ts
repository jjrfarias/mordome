...
import { REPORTS_REVENUE_BY_DAY_VIEW } from "@/lib/permissions";
import { buildRevenueByDayRows, summarizeRevenueByDay, type SaleRecord } from "@/lib/reports/sales";
...
export async function GET(request: Request) {
  ...
  if (isLocalAuthEnabled()) {
    ...
    if (!session.permissionKeys.includes(REPORTS_REVENUE_BY_DAY_VIEW)) return Response.json({ error: "Acesso negado." }, { status: 403 });
    ...
    return Response.json({ rows, summary: summarizeRevenueByDay(rows), from: fromDate.toISOString(), to: toDate.toISOString() });
  }
  ...
  const rows = buildRevenueByDayRows(records);
  return Response.json({ rows, summary: summarizeRevenueByDay(rows), from: fromDate.toISOString(), to: toDate.toISOString() });
}
