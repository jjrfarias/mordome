...
import { REPORTS_PERFORMANCE_BY_STAFF_VIEW } from "@/lib/permissions";
import { buildStaffPerformanceRows } from "@/lib/reports/staff-performance";
...
export async function GET(request: Request) {
  ...
  if (isLocalAuthEnabled()) {
    ...
    if (!session.permissionKeys.includes(REPORTS_PERFORMANCE_BY_STAFF_VIEW)) return Response.json({ error: "Acesso negado." }, { status: 403 });
    ...
    const rows = buildStaffPerformanceRows(sales);
  ...
  const sales = await db.sale.findMany({
    where: { organizationId: session.organization.id, establishmentId: session.establishment.id, status: { in: ["COMPLETED", "PARTIALLY_REFUNDED"] }, completedAt: { gte: fromDate, lte: toDate }, channel: { in: ["POS", "FLOOR"] } },
    include: { payments: true, refunds: true, tab: { include: { table: true } }, operator: true },
  ...
  const rows = buildStaffPerformanceRows(records);
  return Response.json({ rows, from: fromDate.toISOString(), to: toDate.toISOString() });
}
