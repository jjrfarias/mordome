...
import { REPORTS_PAYMENT_METHODS_VIEW } from "@/lib/permissions";
import { buildPaymentMethodsRows, summarizePaymentMethods } from "@/lib/reports/payment-methods";
...
export async function GET(request: Request) {
  ...
  if (isLocalAuthEnabled()) {
    ...
    if (!session.permissionKeys.includes(REPORTS_PAYMENT_METHODS_VIEW)) return Response.json({ error: "Acesso negado." }, { status: 403 });
    ...
    const rows = buildPaymentMethodsRows(sales);
    return Response.json({ rows, summary: summarizePaymentMethods(rows), from: fromDate.toISOString(), to: toDate.toISOString() });
  }
  ...
  const rows = buildPaymentMethodsRows(records);
  return Response.json({ rows, summary: summarizePaymentMethods(rows), from: fromDate.toISOString(), to: toDate.toISOString() });
}
