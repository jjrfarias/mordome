...
export default function OnlineOrderPage() {
  ...
  const submitOrder = async () => {
    ...
    try {
      const response = await fetch(`/api/public/orders/${params.establishmentId}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerName: customerName.trim(), customerPhone: customerPhone.trim(), address: address.trim(), destinationLat: point?.lat, destinationLng: point?.lng, notes: notes.trim() || undefined, deliveryAreaId: deliveryAreaId || undefined, items: items.map(item => ({ productId: item.product.id, quantity: item.quantity })) }) });
      const json = await response.json().catch(() => ({}));
  ...
  return <div className="public-menu" style={{ paddingBottom: items.length > 0 ? 110 : 60 }}>
    ...
    {step === "catalog" && (data.products.length === 0 ? <div className="big-empty"><UtensilsCrossed /><h2>Pedidos indisponíveis</h2><p>Nenhum produto disponível para pedido online no momento.</p></div> : <>
      ...
      <div className="public-menu-layout">
        ...
        <aside className="public-menu-sidebar">
          <h2>Sua sacola</h2>
          {items.length === 0 ? <p style={{ fontSize: 12, color: "var(--muted)" }}>Toque em um produto para começar.</p> : <>
    ...
    {step === "catalog" && items.length > 0 && <div className="public-menu-cartbar" style={{ position: "fixed", left: 0, right: 0, bottom: 0, background: "var(--paper)", borderTop: "1px solid var(--line)", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, maxWidth: 640, margin: "0 auto" }}>
