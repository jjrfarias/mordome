...
import { DashboardsWorkspace } from "@/components/admin/DashboardsWorkspace";
import { DASHBOARDS_REGISTRY } from "@/lib/dashboards/registry";
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav>
        ...
        {DASHBOARDS_REGISTRY.some(dashboard => session.permissionKeys.includes(dashboard.permissionKey)) && <NavItem active={view === "dashboards"} icon={<LayoutDashboard />} label="Dashboards" onClick={() => { setView("dashboards"); }} />}
...
function Pos({ establishmentId, onFinish }: { establishmentId: string; onFinish: (items: OrderItem[], checkout: SaleCheckout) => Promise<boolean> }) {
  ...
  const [couponCode, setCouponCode] = useState(""); const [appliedCoupon, setAppliedCoupon] = useState<string | null>(null); const [couponError, setCouponError] = useState(""); const [applyingCoupon, setApplyingCoupon] = useState(false);
  const total = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
  // Cupom (ADR 0041): forma alternativa e assistida de preencher o desconto que já existe no
  // fluxo de venda — ao validar, aplica automaticamente em `discount`/`discountReason`. O código
  // vai junto na finalização (`couponCode`) para o servidor recalcular e registrar o uso.
  const applyCoupon = async () => {
    if (!couponCode.trim() || !total) return;
    setApplyingCoupon(true); setCouponError("");
    try {
      const response = await fetch("/api/operations/coupons/validate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: couponCode.trim(), subtotal: total }) });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error ?? "Cupom inválido.");
      setDiscount(String(data.discount).replace(".", ","));
      setDiscountReason(`Cupom ${data.coupon.code}`);
      setAppliedCoupon(data.coupon.code as string);
    } catch (cause) { setCouponError(cause instanceof Error ? cause.message : "Cupom inválido."); setAppliedCoupon(null); } finally { setApplyingCoupon(false); }
  };
  const clearCoupon = () => { setAppliedCoupon(null); setCouponCode(""); setCouponError(""); setDiscount(""); setDiscountReason(""); };
  ...
  return <div className="pos-layout">
    ...
    <aside className="pos-cart"><div className="pos-cart-head"><div><span>VENDA ATUAL</span><h2>{cart.reduce((sum, item) => sum + item.quantity, 0)} itens</h2></div>{cart.length > 0 && <button onClick={() => setCart([])}>Limpar</button>}</div><div className="pos-cart-items">{cart.length === 0 ? <div className="empty"><ShoppingBag/><b>Nenhum produto</b><span>Toque em um produto para começar a venda.</span></div> : cart.map(item => <div className="pos-cart-row" key={item.cartLineId}><span className="food">{item.imageUrl ? <img src={item.imageUrl} alt="" /> : item.emoji}</span><div><b>{item.name}</b>{item.selectedOptions?.length ? <small className="pos-cart-options">{item.selectedOptions.map(option => option.optionName).join(", ")}</small> : null}<small>{money(item.price * item.quantity)}</small></div><div className="stepper"><button onClick={() => change(item.cartLineId!, -1)}><Minus/></button><b>{item.quantity}</b><button onClick={() => change(item.cartLineId!, 1)}><Plus/></button></div></div>)}</div>
      <div className="pos-coupon">
        <label className="field"><span>Cupom</span><input value={couponCode} onChange={event => setCouponCode(event.target.value.toUpperCase())} placeholder="Ex.: BEMVINDO10" disabled={!!appliedCoupon} maxLength={40} /></label>
        {appliedCoupon ? <button type="button" className="secondary" onClick={clearCoupon}>Remover cupom {appliedCoupon}</button> : <button type="button" className="secondary" disabled={!couponCode.trim() || !cart.length || applyingCoupon} onClick={() => void applyCoupon()}>{applyingCoupon ? "Validando…" : "Aplicar cupom"}</button>}
        {couponError && <small className="auth-error">{couponError}</small>}
      </div>
      <div className="pos-payment"><PaymentComposer grossTotal={total} discount={discount} setDiscount={setDiscount} discountReason={discountReason} setDiscountReason={setDiscountReason} payments={payments} setPayments={setPayments}/><button className="primary wide" disabled={!cart.length || finishing} onClick={async () => { setFinishing(true); const completed = await onFinish(cart, { ...serializeCheckout(payments, discount, discountReason, total), couponCode: appliedCoupon ?? undefined }); setFinishing(false); if (completed) { setCart([]); setDiscount(""); setDiscountReason(""); setPayments([{ method: "Pix", amount: "", receivedAmount: "" }]); clearCoupon(); } }}><CircleDollarSign/> {finishing ? "Finalizando…" : "Finalizar venda"}</button><small className="shortcut-hint">Atalho: pressione <kbd>F8</kbd> para finalizar</small></div></aside>
