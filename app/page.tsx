...
type View = "pdv" | "salão" | "cozinha" | "delivery" | "entregas" | "caixa" | "resumo" | "historico" | "config";
type AuthSession = { user: { name: string; username: string }; organization: { name: string }; establishment: { id: string; name: string }; establishments: { id: string; name: string }[]; canManageEstablishments: boolean; canManageCatalog: boolean; canManageStock: boolean; canManageRecipes: boolean; canSellPos: boolean; canCancelSales: boolean; canRefundSales: boolean; canApplyDiscount: boolean; canOverrideDiscount: boolean; canOperateFloor: boolean; canManageFloor: boolean; canOperateDelivery: boolean; canDeliverOrders: boolean; canCancelSentItems: boolean; canOpenCash: boolean; canMoveCash: boolean; canCloseCash: boolean; canViewCashHistory: boolean; canViewAudit: boolean; canViewFinanceSummary: boolean; canViewUsers: boolean; canCreateUsers: boolean; canDisableUsers: boolean; canResetUserPassword: boolean; canManageRoles: boolean; canManageIntegrations: boolean; canReprint: boolean; printerDriver: string };
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {view === "config" && (session.canManageEstablishments || session.canManageCatalog || session.canManageStock || session.canManageRecipes || session.canManageFloor || session.canViewUsers || session.canManageRoles || session.canManageIntegrations) && <SettingsWorkspace activeEstablishmentId={session.establishment.id} activeEstablishmentName={session.establishment.name} canManageEstablishments={session.canManageEstablishments} canManageCatalog={session.canManageCatalog} canManageStock={session.canManageStock} canManageRecipes={session.canManageRecipes} canViewUsers={session.canViewUsers} canCreateUsers={session.canCreateUsers} canDisableUsers={session.canDisableUsers} canResetUserPassword={session.canResetUserPassword} canManageRoles={session.canManageRoles} canManageIntegrations={session.canManageIntegrations} canManageFloor={session.canManageFloor} onChanged={refreshSession} />}
    </main>
    <MobileNav view={view} setView={setView} canSellPos={session.canSellPos} canOperateFloor={session.canOperateFloor} canOperateDelivery={session.canOperateDelivery} canDeliverOrders={session.canDeliverOrders} canViewFinanceSummary={session.canViewFinanceSummary} canManageEstablishments={session.canManageEstablishments || session.canManageCatalog || session.canManageStock || session.canManageRecipes || session.canManageFloor || session.canViewUsers || session.canManageRoles || session.canManageIntegrations} />
